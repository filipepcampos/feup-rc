...
int main(int argc, char** argv)
{
    ...
    system("socat -d -d PTY,link=/dev/ttyS0,mode=777 PTY,link=/dev/emulatorT2,mode=777 &");
    sleep(1);
    printf("\n");

    system("socat -d -d PTY,link=/dev/ttyS1,mode=777 PTY,link=/dev/emulatorR2,mode=777 &");
    sleep(1);


    printf( "\n \n"
            "Transmitter must open /dev/ttyS0 \n"
            "Receiver must open /dev/ttyS1 \n \n"
    ...
    fdTx = open("/dev/emulatorT2", O_RDWR | O_NOCTTY );
    if (fdTx <0) {perror("/dev/emulatorT2"); exit(-1); }
    ...
    fdRx = open("/dev/emulatorR2", O_RDWR | O_NOCTTY );
    if (fdRx <0) {perror("/dev/emulatorR2"); exit(-1); }
